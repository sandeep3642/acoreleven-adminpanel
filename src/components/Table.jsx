...
const DataTable = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showActionMenu !== null) {
        const dropdownElement = dropdownRefs.current[showActionMenu];
  ...
  const ActionDropdown = ({ row, onClose, handleClick, index, isLastRows = false }) => (
    <div 
      ref={el => dropdownRefs.current[`${row.id || 'row'}_${index}`] = el}
      className={`absolute ${isLastRows ? 'bottom-8' : 'top-8'} right-0 bg-white border border-gray-200 rounded-lg shadow-lg z-50 min-w-32`}
    >
      {actionMenu &&
        actionMenu.length > 0 &&
        actionMenu.map((val) => (
  ...
  const MobileCard = ({ row, index }) => {
    const menuKey = `${row.id || 'row'}_${index}`;
    const isActiveRow = showActionMenu === menuKey;
        ...
        <div className="space-y-2">
          {headers.map((header) => (
            <div key={header.key} className="flex justify-between items-start">
              ...
              <span className={`text-sm text-right ml-2 flex-1 max-w-[60%] ${isActiveRow ? 'text-[#D50C0C]' : 'text-[#121212]'}`}>
              ...
              <div className="relative">
                ...
                {showActionMenu === menuKey && (
                  <ActionDropdown
                    ...
                    isLastRows={index >= data.length - 2}
                  />
                )}
              </div>
            </div>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.length === 0 ? (
                ...
                data.map((row, index) => {
                  const menuKey = `${row.id || 'row'}_${index}`;
                  const isLastRows = index >= data.length - 2; // Check if it's one of the last 2 rows
                  const isActiveRow = showActionMenu === menuKey;
                  ...
                  return (
                    <tr key={row.id || index} className="hover:bg-gray-50">
                      {headers.map((header) => (
                        <td
                          key={header.key}
                          className={`px-6 py-4 whitespace-nowrap text-sm ${isActiveRow ? 'text-[#D50C0C]' : 'text-[#121212]'}`}
                        >
